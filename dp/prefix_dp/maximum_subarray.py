...
class Solution:  # find the contiguous subarray(len >= 1) that has the largest sum and return its sum
    def maxSubArray(self, nums: list[int]) -> int:
        ...
        for i in range(1, len(nums)):
            f[i] = max(f[i - 1] + nums[i], nums[i])  # either include new num or start over from i
    ...
    def maxSubArray2(self, nums: list[int]) -> int:  # rolling variable O(1) space
        if not nums:
            return 0
        max_sum_at_i = nums[0]
        max_sum = max_sum_at_i
        for i in range(1, len(nums)):
            max_sum_at_i = max(max_sum_at_i + nums[i], nums[i])
            max_sum = max(max_sum, max_sum_at_i)
        return max_sum
