class Solution:  # https://leetcode-cn.com/problems/counting-bits/
    ...
    def countBits(self, n: int) -> List[int]:  # given int n, 返回n+1长度数组: 位置i为i的hamming weight(二进制表示中1的个数)  O(nlogn)
        return [self.hamming_weight(i) for i in range(n + 1)]

    def countBits1(self, n: int) -> List[int]:  # O(n) O(1)  dynamic programming
        ones = [0]  # ones[i] = i的二进制中1的个数
        for i in range(1, n + 1):  # 当前i二进制中1的个数 = ones[i >> 1]最后一位之前的1的个数 + i & 1 最后位是否为1
